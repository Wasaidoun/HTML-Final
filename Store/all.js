document.addEventListener("DOMContentLoaded", () => {
    // Image selection and details display logic
    ...
    function forgotpass(event) {
        ...
        const out1 = document.getElementById("out1"); // Output message element

        if (emailInput && emailInput.value.trim()) {
            // Display confirmation message if a valid email is entered
            out1.textContent = `Email sent to: ${emailInput.value.trim()}`;
        } else {
            // Display error message if the input is empty
            out1.textContent = "Please enter a valid email address.";
        }
    }

    // Attach event listener to the form
    const forgotForm = document.getElementById("forgotPasswordForm"); // Get the form element
    if (forgotForm) {
